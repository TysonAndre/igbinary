...
zend_always_inline static int igbinary_unserialize_zval(struct igbinary_unserialize_data *igsd, uint32_t version, zval *const z, int flags) {
	if (version == 3) {
		return igbinary_unserialize_v3_zval_wrapper(igsd, z, flags);
	} else {
...
IGBINARY_API int igbinary_serialize_ex(uint8_t **ret, size_t *ret_len, zval *z, struct igbinary_memory_manager *memory_manager) {
	...
	if (use_v3) {
		if (UNEXPECTED(igbinary_serialize_v3_zval_wrapper(&igsd, z) != 0)) {
			return_code = 1;
			goto cleanup;
		}
	} else {
...
PS_SERIALIZER_ENCODE_FUNC(igbinary)
{
	...
	bool failed;
	if (use_v3) {
		failed = igbinary_serialize_v3_array_wrapper(&igsd, session_vars, false, false, true) != 0;
	} else {
