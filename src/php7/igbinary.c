...
enum igbinary_v3_type {
	...
	/* 00 */ igbinary_v3_type_undef,			/**< Undefined. E.g. a declared instance property that was omitted from serialization */
...
zend_always_inline static int igbinary_unserialize_header(struct igbinary_unserialize_data *igsd, const uint8_t version);
...
PHP_INI_BEGIN()
	...
	/* This is new and still experimental, and marked as off when serializing data */
	STD_PHP_INI_BOOLEAN("igbinary.use_v3_serialize_format", "0", PHP_INI_ALL, OnUpdateBool, use_v3_serialize_format, zend_igbinary_globals, igbinary_globals)
PHP_INI_END()
/* }}} */
...
static void php_igbinary_init_globals(zend_igbinary_globals *igbinary_globals) {
	...
	igbinary_globals->use_v3_serialize_format = 0;
}
/* }}} */
...
PHP_MINFO_FUNCTION(igbinary) {
	...
	/* Ini settings are printed by php already in phpinfo */
...
IGBINARY_API int igbinary_serialize_ex(uint8_t **ret, size_t *ret_len, zval *z, struct igbinary_memory_manager *memory_manager) {
	...
	const bool use_v3 = IGBINARY_G(use_v3_serialize_format);
	igbinary_serialize_header(&igsd, use_v3 ? IGBINARY_FORMAT_VERSION_V3 : IGBINARY_FORMAT_VERSION_V2);
	return_code = 0;

	if (use_v3) {
		if (UNEXPECTED(igbinary_serialize_v3_zval(&igsd, z) != 0)) {
			return_code = 1;
			goto cleanup;
		}
	} else {
		if (UNEXPECTED(igbinary_serialize_v2_zval(&igsd, z) != 0)) {
			return_code = 1;
			goto cleanup;
		}
	}
	...
	/* NOTE: zend_string also adds explicit null termination */
...
IGBINARY_API int igbinary_unserialize(const uint8_t *buf, size_t buf_len, zval *z) {
	...
	const int version = igbinary_unserialize_header(&igsd);
	if (UNEXPECTED(!version)) {
		ret = 1;
		goto cleanup;
	}
	if (UNEXPECTED(version != 3)) {
		zend_error(E_WARNING, "igbinary_unserialize: todo reimplement version 2");
		ret = 1;
		goto cleanup;
	}

	if (UNEXPECTED(igbinary_unserialize_v3_zval(&igsd, z, WANT_CLEAR))) {
		ret = 1;
		goto cleanup;
	}
...
PS_SERIALIZER_ENCODE_FUNC(igbinary)
{
	...
	bool failed;
	if (use_v3) {
		failed = igbinary_serialize_v3_array(&igsd, session_vars, false, false, true) != 0;
	} else {
		failed = igbinary_serialize_v2_array(&igsd, session_vars, false, false, true) != 0;
	}
...
/** Serializes header ("\x00\x00\x00\x02" or "\x00\x00\x00\x03"). */
inline static void igbinary_serialize_header(struct igbinary_serialize_data *igsd, uint8_t version) {
...
inline static int igbinary_serialize_v3_string(struct igbinary_serialize_data *igsd, zend_string *s) {
	...
	if (len == 0) {
		/* The empty string is always serialized as igbinary_serialize_v3_string (1 byte instead of 2) */
...
#if SIZEOF_SIZE_T > 4
static zend_never_inline int igbinary_serialize_v3_extremely_long_chararray(struct igbinary_serialize_data *igsd, const char *s, size_t len) {
	RETURN_1_IF_NON_ZERO(igbinary_serialize_resize(igsd, len + 9));
	uint8_t *append_buffer = &igsd->buffer[igsd->buffer_size];
	append_buffer[0] = igbinary_v3_type_string64;
...
inline static int igbinary_serialize_v3_chararray(struct igbinary_serialize_data *igsd, const char *s, size_t len) {
	...
	if (len <= 0xff) {
		...
		if (UNEXPECTED(len > 0xffffffff)) {
			return igbinary_serialize_v3_extremely_long_chararray(igsd, s, len);
		}
#endif
...
/* }}} */
/* igbinary_unserialize_v3_extremely_long_chararray for 4GB+ of data {{{ */
static zend_never_inline zend_string* igbinary_unserialize_v3_extremely_long_chararray(struct igbinary_unserialize_data *igsd) {
#if SIZEOF_ZEND_LONG == 4
	(void) igsd;
	zend_error(E_WARNING, "igbinary_unserialize_v3_chararray: cannot unserialize 64-bit data on 32-bit platform");
	return NULL;
#else
	if (IGB_NEEDS_MORE_DATA(igsd, 8)) {
...
inline static zend_string *igbinary_unserialize_v3_chararray(struct igbinary_unserialize_data *igsd, enum igbinary_v3_type t, zend_bool check_interned) {
	...
	if (t == igbinary_v3_type_string8 || t == igbinary_v3_type_object8) {
		...
		*/
		...
		return igbinary_unserialize_v3_extremely_long_chararray(igsd);
	} else {
...
zend_always_inline static int igbinary_unserialize_v3_array(struct igbinary_unserialize_data *igsd, enum igbinary_v3_type t, zval *const z, int flags, zend_bool create_ref) {
	...
	/* FIXME remove assertion, only there for data created by igbinary, not for user-provided data */
	ZEND_ASSERT(n > 0);
...
inline static int igbinary_unserialize_v3_object_properties(struct igbinary_unserialize_data *igsd, enum igbinary_v3_type t, zval *const z, const zend_class_entry *ce) {
	...
	/* FIXME remove, only testing the serializer */
	ZEND_ASSERT(n > 0);
...
enum igbinary_v2_type {
	...
	/* 01 */ igbinary_v2_type_ref8,				/**< Array reference. */
	/* 02 */ igbinary_v2_type_ref16,			/**< Array reference. */
	/* 03 */ igbinary_v2_type_ref32,			/**< Array reference. */

	/* 04 */ igbinary_v2_type_bool_false,		/**< Boolean true. */
	/* 05 */ igbinary_v2_type_bool_true,		/**< Boolean false. */

	/* 06 */ igbinary_v2_type_long8p,			/**< Long 8bit positive. */
	/* 07 */ igbinary_v2_type_long8n,			/**< Long 8bit negative. */
	/* 08 */ igbinary_v2_type_long16p,			/**< Long 16bit positive. */
	/* 09 */ igbinary_v2_type_long16n,			/**< Long 16bit negative. */
	/* 0a */ igbinary_v2_type_long32p,			/**< Long 32bit positive. */
	/* 0b */ igbinary_v2_type_long32n,			/**< Long 32bit negative. */

	/* 0c */ igbinary_v2_type_double,			/**< Double. */

	/* 0d */ igbinary_v2_type_string_empty,		/**< Empty string. */

	/* 0e */ igbinary_v2_type_string_id8,		/**< String id. */
	/* 0f */ igbinary_v2_type_string_id16,		/**< String id. */
	/* 10 */ igbinary_v2_type_string_id32,		/**< String id. */

	/* 11 */ igbinary_v2_type_string8,			/**< String. */
	/* 12 */ igbinary_v2_type_string16,			/**< String. */
	/* 13 */ igbinary_v2_type_string32,			/**< String. */

	/* 14 */ igbinary_v2_type_array8,			/**< Array. */
	/* 15 */ igbinary_v2_type_array16,			/**< Array. */
	/* 16 */ igbinary_v2_type_array32,			/**< Array. */

	/* 17 */ igbinary_v2_type_object8,			/**< Object. */
	/* 18 */ igbinary_v2_type_object16,			/**< Object. */
	/* 19 */ igbinary_v2_type_object32,			/**< Object. */

	/* 1a */ igbinary_v2_type_object_id8,		/**< Object class name string id. */
	/* 1b */ igbinary_v2_type_object_id16,		/**< Object class name string id. */
	/* 1c */ igbinary_v2_type_object_id32,		/**< Object class name string id. */

	/* 1d */ igbinary_v2_type_object_ser8,		/**< Object serialized data. */
	/* 1e */ igbinary_v2_type_object_ser16,		/**< Object serialized data. */
	/* 1f */ igbinary_v2_type_object_ser32,		/**< Object serialized data. */

	/* 20 */ igbinary_v2_type_long64p,			/**< Long 64bit positive. */
	/* 21 */ igbinary_v2_type_long64n,			/**< Long 64bit negative. */

	/* 22 */ igbinary_v2_type_objref8,			/**< Object reference. */
	/* 23 */ igbinary_v2_type_objref16,			/**< Object reference. */
	/* 24 */ igbinary_v2_type_objref32,			/**< Object reference. */

	/* 25 */ igbinary_v2_type_ref,				/**< Simple reference */
	/* 26 */ igbinary_v2_type_string64,			/**< String larger than 4GB (originally, php strings had a limit of 32-bit lengths). */
	/* 27 */ igbinary_v2_type_enum_case,		/**< PHP 8.1 Enum case. */
};
/* }}} */
...
zend_always_inline static void igbinary_serialize_v2_header(struct igbinary_serialize_data *igsd);

zend_always_inline static int igbinary_serialize_v2_null(struct igbinary_serialize_data *igsd);
zend_always_inline static int igbinary_serialize_v2_bool(struct igbinary_serialize_data *igsd, int b);
zend_always_inline static int igbinary_serialize_v2_long(struct igbinary_serialize_data *igsd, zend_long l);
zend_always_inline static int igbinary_serialize_v2_double(struct igbinary_serialize_data *igsd, double d);
zend_always_inline static int igbinary_serialize_v2_string(struct igbinary_serialize_data *igsd, zend_string *s);
zend_always_inline static int igbinary_serialize_v2_chararray(struct igbinary_serialize_data *igsd, const char *s, size_t len);

zend_always_inline static int igbinary_serialize_v2_array(struct igbinary_serialize_data *igsd, zval *z, bool object, bool incomplete_class, bool serialize_props);
zend_always_inline static int igbinary_serialize_v2_array_ref(struct igbinary_serialize_data *igsd, zval *z, bool object);
zend_always_inline static int igbinary_serialize_v2_array_sleep(struct igbinary_serialize_data *igsd, zval *z, HashTable *ht, zend_class_entry *ce);
zend_always_inline static int igbinary_serialize_v2_object_name(struct igbinary_serialize_data *igsd, zend_string *name);
zend_always_inline static int igbinary_serialize_v2_object(struct igbinary_serialize_data *igsd, zval *z);

static int igbinary_serialize_zval(struct igbinary_serialize_data *igsd, zval *z);

static int igbinary_unserialize_v2_zval(struct igbinary_unserialize_data *igsd, zval *const z, int flags);
static int igbinary_serialize_v2_zval(struct igbinary_serialize_data *igsd, zval *z);

/* {{{ igbinary_serialize_v2_null */
/** Serializes null. */
inline static int igbinary_serialize_v2_null(struct igbinary_serialize_data *igsd) {
	return igbinary_serialize_v28(igsd, igbinary_v2_type_null);
}
/* }}} */
/* {{{ igbinary_serialize_v2_bool */
/** Serializes bool. */
inline static int igbinary_serialize_v2_bool(struct igbinary_serialize_data *igsd, int b) {
	return igbinary_serialize_v28(igsd, (uint8_t)(b ? igbinary_v2_type_bool_true : igbinary_v2_type_bool_false));
}
/* }}} */
/* {{{ igbinary_serialize_v2_long */
/** Serializes zend_long. */
inline static int igbinary_serialize_v2_long(struct igbinary_serialize_data *igsd, zend_long l) {
	const bool p = l >= 0;
	// k is the absolute value of l
	const zend_ulong k = p ? (zend_ulong)l : -(zend_ulong)l;

	if (k <= 0xff) {
		RETURN_1_IF_NON_ZERO(igbinary_serialize_v28_and_8(igsd,
			p ? igbinary_v2_type_long8p : igbinary_v2_type_long8n,
			(uint8_t)k
		));
	} else if (k <= 0xffff) {
		RETURN_1_IF_NON_ZERO(igbinary_serialize_v28_and_16(igsd,
			p ? igbinary_v2_type_long16p : igbinary_v2_type_long16n,
			(uint16_t)k
		));
#if SIZEOF_ZEND_LONG == 8
	} else if (k <= 0xffffffff) {
		RETURN_1_IF_NON_ZERO(igbinary_serialize_v28_and_32(igsd,
			p ? igbinary_v2_type_long32p : igbinary_v2_type_long32n,
			(uint32_t)k
		));
	} else {
		RETURN_1_IF_NON_ZERO(igbinary_serialize_v28_and_64(igsd,
			p ? igbinary_v2_type_long64p : igbinary_v2_type_long64n,
			(uint64_t)k
		));
	}
#elif SIZEOF_ZEND_LONG == 4
...
/* }}} */
/* {{{ igbinary_serialize_v2_double */
/** Serializes double. */
inline static int igbinary_serialize_v2_double(struct igbinary_serialize_data *igsd, double d) {
	union {
		double d;
		uint64_t u;
	} u;
	...
	return igbinary_serialize_v28_and_64(igsd, igbinary_v2_type_double, u.u);
}
/* }}} */
/* {{{ igbinary_serialize_v2_string */
/**
 * Serializes string.
 *
 * When compact_strings is true,
 * this will serialize the string as igbinary_v2_type_string* (A length followed by a character array) the first time,
 * and serialize subsequent references to the same string as igbinary_v2_type_string_id*.
 *
 * When compact_strings is false, this will always serialize the string as a character array.
 * compact_strings speeds up serialization, but slows down serialization and uses more space to represent the serialization.
 *
 * Serializes each string once.
 * After first time uses pointers (igbinary_v2_type_string_id*) instead of igbinary_v2_type_string*.
 */
inline static int igbinary_serialize_v2_string(struct igbinary_serialize_data *igsd, zend_string *s) {
	const size_t len = ZSTR_LEN(s);
	if (len == 0) {
		/* The empty string is always serialized as igbinary_serialize_v2_string (1 byte instead of 2) */
		return igbinary_serialize_v28(igsd, igbinary_v2_type_string_empty);
	}
	...
	if (!igsd->scalar && igsd->compact_strings) {
		...
		if (result.code == hash_si_code_exists) {
			uint32_t value = result.value;
			if (value <= 0xff) {
				RETURN_1_IF_NON_ZERO(igbinary_serialize_v28_and_8(igsd, (uint8_t)igbinary_v2_type_string_id8, (uint8_t)value));
			} else if (value <= 0xffff) {
				RETURN_1_IF_NON_ZERO(igbinary_serialize_v28_and_16(igsd, (uint8_t)igbinary_v2_type_string_id16, (uint16_t)value));
			} else {
				RETURN_1_IF_NON_ZERO(igbinary_serialize_v28_and_32(igsd, (uint8_t)igbinary_v2_type_string_id32, value));
			}
			return 0;
		} else if (EXPECTED(result.code == hash_si_code_inserted)) {
			/* Fall through to igbinary_serialize_v2_chararray */
		} else {
			return 1;  /* Failed to allocate copy of string */
		}
	}
	...
	igsd->string_count++; /* A new string is being serialized - update count so that duplicate class names can be used. */
	if (UNEXPECTED(igsd->string_count == 0)) {
		zend_error(E_WARNING, "igbinary_serialize_v2: Saw too many strings");
		return 1;
	}
	return igbinary_serialize_v2_chararray(igsd, ZSTR_VAL(s), len);
}
/* }}} */

/* {{{ igbinary_serialize_v2_chararray */
/** Serializes string data as the type followed by the length followed by the raw character array. */
inline static int igbinary_serialize_v2_chararray(struct igbinary_serialize_data *igsd, const char *s, size_t len) {
	uint8_t *append_buffer;
	int offset;
	if (len <= 0xff) {
		RETURN_1_IF_NON_ZERO(igbinary_serialize_v2_resize(igsd, len + 2));
		append_buffer = &igsd->buffer[igsd->buffer_size];
		append_buffer[0] = igbinary_v2_type_string8;
		append_buffer[1] = len;
		offset = 2;
	} else if (len <= 0xffff) {
		RETURN_1_IF_NON_ZERO(igbinary_serialize_v2_resize(igsd, len + 3));
		append_buffer = &igsd->buffer[igsd->buffer_size];
		append_buffer[0] = igbinary_v2_type_string16;
		append_buffer[1] = (uint8_t)(len >> 8 & 0xff);
		append_buffer[2] = (uint8_t)(len & 0xff);
		offset = 3;
	} else {
#if SIZEOF_SIZE_T > 4
		if (UNEXPECTED(len > 0xffffffff)) {
			return igbinary_serialize_v2_extremely_long_chararray(igsd, s, len);
		}
#endif
		RETURN_1_IF_NON_ZERO(igbinary_serialize_v2_resize(igsd, len + 5));
		append_buffer = &igsd->buffer[igsd->buffer_size];
		append_buffer[0] = igbinary_v2_type_string32;
		append_buffer[1] = (uint8_t)(len >> 24 & 0xff);
		append_buffer[2] = (uint8_t)(len >> 16 & 0xff);
		append_buffer[3] = (uint8_t)(len >> 8 & 0xff);
		append_buffer[4] = (uint8_t)(len & 0xff);
...
/* }}} */
/* {{{ igbinary_serialize_v2_array */
/**
 * Serializes an array's or object's inner properties.
 * If properties or keys are unexpectedly added (e.g. by __sleep() or serialize() elsewhere), this will skip serializing them.
 * If properties or keys are unexpectedly removed, this will add igbinary_v2_type_null as padding for the corresponding entries.
 */
inline static int igbinary_serialize_v2_array(struct igbinary_serialize_data *igsd, zval *z, bool object, bool incomplete_class, bool serialize_props) {
	/* If object=true: z is IS_OBJECT */
	/* If object=false: z is either IS_ARRAY, or IS_REFERENCE pointing to an IS_ARRAY. */
	HashTable *h;
	/* At the time of writing, struct _zend_array had uint32_t nNumOfElements */
	...
	if (object) {
		...
		/* if it is an array or a reference to an array, then add a reference unique to that **reference** to that array */
		if (serialize_props) {
			int ref_ser = igbinary_serialize_v2_array_ref(igsd, z_original, false);
			if (ref_ser != 2) {
				/* 1 means out of memory or error, 0 means this already exists, 2 means this is new. */
				return ref_ser;
			}
	...
	if (n <= 0xff) {
		RETURN_1_IF_NON_ZERO(igbinary_serialize_v28_and_8(igsd, igbinary_v2_type_array8, (uint8_t)n));
		...
		RETURN_1_IF_NON_ZERO(igbinary_serialize_v28_and_16(igsd, igbinary_v2_type_array16, (uint16_t)n));
	} else {
		RETURN_1_IF_NON_ZERO(igbinary_serialize_v28_and_32(igsd, igbinary_v2_type_array32, n));
	}
	...
	ZEND_HASH_FOREACH_KEY_VAL(h, key_index, key, d) {
		...
		if (Z_TYPE_P(d) == IS_UNDEF) {
			...
			/* This can't be a value or a reference in an array - except maybe for $GLOBALS, which has other issues. */
			ZEND_ASSERT(!serialize_props);
			RETURN_1_IF_NON_ZERO(igbinary_serialize_v2_null(igsd));
			continue;
		}

		if (key == NULL) {
			/* Key is numeric */
			RETURN_1_IF_NON_ZERO(igbinary_serialize_v2_long(igsd, key_index));
		} else {
			/* Key is string */
			RETURN_1_IF_NON_ZERO(igbinary_serialize_v2_string(igsd, key));
		}

		/* we should still add element even if it's not OK,
		 * since we already wrote the length of the array before */
		RETURN_1_IF_NON_ZERO(igbinary_serialize_v2_zval(igsd, d));
	} ZEND_HASH_FOREACH_END();
	if (object) {
		zend_release_properties(h);
	}
...
/* }}} */
/* {{{ igbinary_serialize_v2_array_ref */
/** Serializes array reference (or reference in an object). Returns 0 on success. */
/* TODO: Use different result codes for missing keys and errors */
zend_always_inline static int igbinary_serialize_v2_array_ref(struct igbinary_serialize_data *igsd, zval * const z, bool object) {
	size_t t;
	zend_uintptr_t key;  /* The address of the pointer to the zend_refcounted struct or other struct */
	static int INVALID_KEY;  /* Not used, but we take the pointer of this knowing other zvals won't share it*/
...
// 		/* Increment the reference id for the deserializer, give up. */
// 		++igsd->references_id;
// 		php_error_docref(NULL, E_NOTICE, "igbinary_serialize_v2_array_ref expected either object or reference (param object=%s), got neither (zend_type=%d)", object ? "true" : "false", (int)Z_TYPE_P(z));
// 		return 1;
// 	}
	...
	if (t == SIZE_MAX) {
		...
		igsd->references_id++;
		if (UNEXPECTED(igsd->references_id == 0)) {
			zend_error(E_WARNING, "igbinary_serialize_v2: Saw too many references");
			return 1;
		}
		/* We only need to call this if the array/object is new, in case __serialize or other methods return temporary arrays or modify arrays that were serialized earlier */
		igsd_addref_and_defer_dtor(&igsd->deferred_dtor_tracker, z);
	...
	/* TODO: Properly handle running out of memory in this helper function. */
	/* It returns 1 both for running out of memory and inserting a new entry. */
	enum igbinary_v2_type type;
	if (t <= 0xff) {
		type = object ? igbinary_v2_type_objref8 : igbinary_v2_type_ref8;
		RETURN_1_IF_NON_ZERO(igbinary_serialize_v28_and_8(igsd, (uint8_t)type, (uint8_t)t));
	} else if (t <= 0xffff) {
		type = object ? igbinary_v2_type_objref16 : igbinary_v2_type_ref16;
		RETURN_1_IF_NON_ZERO(igbinary_serialize_v28_and_16(igsd, (uint8_t)type, (uint16_t)t))
	} else {
		type = object ? igbinary_v2_type_objref32 : igbinary_v2_type_ref32;
		RETURN_1_IF_NON_ZERO(igbinary_serialize_v28_and_32(igsd, (uint8_t)type, (uint32_t)t))
	}
	return 0;
}
/* }}} */
/* {{{ igbinary_serialize_v2_array_sleep_single_prop_value */
/** Serializes one value of an object's properties array, for use with the __sleep function. */
inline static int igbinary_serialize_v2_array_sleep_single_prop_value(struct igbinary_serialize_data *igsd, zval *z, zval *v, zend_class_entry *ce, zend_string *prop_name) {
	/* Precondition: All args are non-null */
	if (Z_TYPE_P(v) == IS_INDIRECT) {
		v = Z_INDIRECT_P(v);
		if (UNEXPECTED(Z_TYPE_P(v) == IS_UNDEF)) {
...
serialize_untyped_uninitialized_prop:
			php_error_docref(NULL, E_NOTICE, "\"%s\" returned as member variable from __sleep() but does not exist", ZSTR_VAL(prop_name));
			return igbinary_serialize_v2_null(igsd);
		}
	}
	return igbinary_serialize_v2_zval(igsd, v);
}
/* }}} */
/* {{{ igbinary_serialize_v2_array_sleep_inner */
/** Serializes object's properties array with __sleep -function. */
inline static int igbinary_serialize_v2_array_sleep_inner(struct igbinary_serialize_data *igsd, zval *z, HashTable *h, HashTable *object_properties, zend_class_entry *ce) {
	zval *d;
	zval *v;
	...
	ZEND_HASH_FOREACH_VAL(h, d) {
		...
		if ((v = zend_hash_find(object_properties, prop_name)) != NULL) {
			RETURN_1_IF_NON_ZERO(igbinary_serialize_v2_string(igsd, prop_name));

			RETURN_1_IF_NON_ZERO(igbinary_serialize_v2_array_sleep_single_prop_value(igsd, z, v, ce, prop_name));
		} else {
			zend_string *mangled_prop_name;
			...
			res = igbinary_serialize_v2_string(igsd, mangled_prop_name);
			/* igbinary_serialize_v2_string will increase the reference count. */
			zend_string_release_ex(mangled_prop_name, 0);
...
/* }}} */
/* {{{ igbinary_serialize_v2_array_sleep */
/** Serializes object's properties array with __sleep -function. */
inline static int igbinary_serialize_v2_array_sleep(struct igbinary_serialize_data *igsd, zval *z, HashTable *h, zend_class_entry *ce) {
	HashTable *object_properties;
	uint32_t n = zend_hash_num_elements(h);
	...
	int r = igbinary_serialize_v2_array_sleep_inner(igsd, z, h, object_properties, ce);
	zend_release_properties(object_properties);
	return r;
}
/* }}} */
/* {{{ igbinary_serialize_v2_object_name */
/**
 * Serialize a PHP object's class name.
 * Note that this deliberately ignores the compact_strings setting.
 */
inline static int igbinary_serialize_v2_object_name(struct igbinary_serialize_data *igsd, zend_string *class_name) {
	struct hash_si_result result = hash_si_find_or_insert(&igsd->strings, class_name, igsd->string_count);
	if (result.code == hash_si_code_inserted) {
		const size_t name_len = ZSTR_LEN(class_name);
		igsd->string_count += 1;
		...
		if (name_len <= 0xff) {
			RETURN_1_IF_NON_ZERO(igbinary_serialize_v28_and_8(igsd, igbinary_v2_type_object8, (uint8_t)name_len))
		} else if (EXPECTED(name_len <= 0xffff)) {
			RETURN_1_IF_NON_ZERO(igbinary_serialize_v28_and_16(igsd, igbinary_v2_type_object16, (uint16_t)name_len))
		} else {
#if SIZEOF_SIZE_T > 4
			if (UNEXPECTED(name_len > 0xffffffff)) {
				zend_error(E_WARNING, "igbinary_serialize_v2_object_name: class name does not fit in 32 bits");
				return 1;
			}
#endif

			RETURN_1_IF_NON_ZERO(igbinary_serialize_v28_and_32(igsd, igbinary_v2_type_object32, (uint32_t)name_len))
		}

		RETURN_1_IF_NON_ZERO(igbinary_serialize_v2_resize(igsd, name_len));
		...
		uint32_t value = result.value;
		if (value <= 0xff) {
			RETURN_1_IF_NON_ZERO(igbinary_serialize_v28_and_8(igsd, (uint8_t)igbinary_v2_type_object_id8, (uint8_t)value))
		} else if (value <= 0xffff) {
			RETURN_1_IF_NON_ZERO(igbinary_serialize_v28_and_16(igsd, (uint8_t)igbinary_v2_type_object_id16, (uint16_t)value))
		} else {
			RETURN_1_IF_NON_ZERO(igbinary_serialize_v28_and_32(igsd, (uint8_t)igbinary_v2_type_object_id32, (uint32_t)value))
		}
	} else {
		return 1; /* Failed to allocate copy of string */
	}
...
/* }}} */
/* igbinary_serialize_v2_object_old_serializer_class {{{ */
static ZEND_COLD int igbinary_serialize_v2_object_old_serializer_class(struct igbinary_serialize_data* igsd, zval *z, zend_class_entry *ce) {
	unsigned char *serialized_data = NULL;
	size_t serialized_len;
	int r = 0;

	if (ce->serialize(z, &serialized_data, &serialized_len, (zend_serialize_data *)NULL) == SUCCESS && !EG(exception)) {
		if (UNEXPECTED(igbinary_serialize_v2_object_name(igsd, ce->name) != 0)) {
			goto failure;
		}

		if (serialized_len <= 0xff) {
			if (UNEXPECTED(igbinary_serialize_v28_and_8(igsd, igbinary_v2_type_object_ser8, (uint8_t)serialized_len) != 0)) {
				goto failure;
			}
		} else if (serialized_len <= 0xffff) {
			if (UNEXPECTED(igbinary_serialize_v28_and_16(igsd, (uint8_t)igbinary_v2_type_object_ser16, (uint16_t)serialized_len) != 0)) {
				goto failure;
			}
		} else {
#if SIZEOF_ZEND_LONG > 4
			if (UNEXPECTED(serialized_len > 0xffffffff)) {
				zend_error(E_WARNING, "igbinary_serialize_v2_object_old_serializer_class: serialize() data 4GB or larger is not supported");
				goto failure;
			}
#endif
			if (UNEXPECTED(igbinary_serialize_v28_and_32(igsd, (uint8_t)igbinary_v2_type_object_ser32, (uint32_t)serialized_len) != 0)) {
				goto failure;
			}
		}

		if (UNEXPECTED(igbinary_serialize_v2_resize(igsd, serialized_len))) {
			goto failure;
		}
...
failure:
		...
		/* Serialization callback failed, assume null output */
		r = igbinary_serialize_v2_null(igsd);
	}
...
/* }}} */
/* igbinary_serialize_v2_object_new_serializer {{{ */
#if PHP_VERSION_ID >= 70400
inline static int igbinary_serialize_v2_object_new_serializer(struct igbinary_serialize_data* igsd, zval *z, zend_class_entry *ce) {
	zval retval;
	int r = 0;
	...
	if (igbinary_var_serialize_call_magic_serialize(&retval, z)) {
		...
		if (!EG(exception)) {
			// When does this happen?
			igbinary_serialize_v2_null(igsd);
			return 0;
		}
		return 1;
	}
	...
	// object is false because this is the array returned by __serialize() for the object.
	// TODO: Add tests that this works properly when the same array is reused.
	r = igbinary_serialize_v2_array(igsd, &retval, false, false, false);
	zval_ptr_dtor(&retval);
	if (UNEXPECTED(r)) {
		return 1;
	}
...
#endif
/* }}} */
/* {{{ igbinary_serialize_v2_object_enum_case */
#if PHP_VERSION_ID >= 80100
inline static int igbinary_serialize_v2_object_enum_case(struct igbinary_serialize_data *igsd, zend_object *obj, zend_class_entry *ce) {
	if (UNEXPECTED(igbinary_serialize_v2_object_name(igsd, ce->name) != 0)) {
		return 1;
	}
	if (UNEXPECTED(igbinary_serialize_v28(igsd, (uint8_t)igbinary_v2_type_enum_case))) {
		return 1;
	}
	zval *case_name = zend_enum_fetch_case_name(obj);
	ZEND_ASSERT(Z_TYPE_P(case_name) == IS_STRING);
	return igbinary_serialize_v2_string(igsd, Z_STR_P(case_name));
}
#endif
/* }}} */
/* {{{ igbinary_serialize_v2_object */
/** Serialize object.
 * @see ext/standard/var.c
 * */
inline static int igbinary_serialize_v2_object(struct igbinary_serialize_data *igsd, zval *z) {
	PHP_CLASS_ATTRIBUTES;
	...
	int ref_ser = igbinary_serialize_v2_array_ref(igsd, z, true);
	if (ref_ser != 2) {
		return ref_ser;
	}
...
#if PHP_VERSION_ID >= 80100
	if (ce->ce_flags & ZEND_ACC_ENUM) {
		return igbinary_serialize_v2_object_enum_case(igsd, Z_OBJ_P(z), ce);
	}
#endif
	...
	{
		// fprintf(stderr, "Going to serialize %s\n", ZSTR_VAL(ce->name));
		return igbinary_serialize_v2_object_new_serializer(igsd, z, ce);
	}
#endif
	if (ce->serialize != NULL) {
		return igbinary_serialize_v2_object_old_serializer_class(igsd, z, ce);
	}

	/* serialize class name */
	PHP_SET_CLASS_ATTRIBUTES(z);
	if (igbinary_serialize_v2_object_name(igsd, class_name) != 0) {
		PHP_CLEANUP_CLASS_ATTRIBUTES();
		return 1;
	}
	PHP_CLEANUP_CLASS_ATTRIBUTES();
	...
	if (ce && ce != PHP_IC_ENTRY && zend_hash_str_exists(&ce->function_table, "__sleep", sizeof("__sleep") - 1)) {
		...
		if (r == SUCCESS && !EG(exception)) {
			...
			if (Z_TYPE(h) == IS_UNDEF) {
				...
				/* NOTE: PHP permits returning an object in __sleep */
				r = igbinary_serialize_v2_array_sleep(igsd, z, ht, ce);
			} else {
				php_error_docref(NULL, E_NOTICE, "__sleep should return an array only "
						"containing the names of instance-variables to "
						"serialize");
				...
				/* empty array */
				r = igbinary_serialize_v28_and_8(igsd, igbinary_v2_type_array8, 0);
			}
		} else {
			r = 1;
		}
	...
	return igbinary_serialize_v2_array(igsd, z, true, incomplete_class, false);
}
/* }}} */
/* {{{ igbinary_serialize_v2_zval */
/** Serialize zval. */
static int igbinary_serialize_v2_zval(struct igbinary_serialize_data *igsd, zval *z) {
	if (Z_ISREF_P(z)) {
		if (Z_REFCOUNT_P(z) >= 2) {
			RETURN_1_IF_NON_ZERO(igbinary_serialize_v28(igsd, (uint8_t)igbinary_v2_type_ref))

			switch (Z_TYPE_P(Z_REFVAL_P(z))) {
			case IS_ARRAY:
				return igbinary_serialize_v2_array(igsd, z, false, false, true);
			case IS_OBJECT:
				break; /* Fall through */
			default:
				{
					/* Serialize a reference if zval already added */
					int ref_ser = igbinary_serialize_v2_array_ref(igsd, z, false);
					if (ref_ser != 2) {
						return ref_ser;
					}
					/* Fall through */
	...
	switch (Z_TYPE_P(z)) {
		case IS_RESOURCE:
			...
			return igbinary_serialize_v2_null(igsd);
		case IS_OBJECT:
			return igbinary_serialize_v2_object(igsd, z);
		case IS_ARRAY:
			/* if is_ref, then php5 would have called igbinary_serialize_v2_array_ref */
			return igbinary_serialize_v2_array(igsd, z, false, false, true);
		case IS_STRING:
			return igbinary_serialize_v2_string(igsd, Z_STR_P(z));
		case IS_LONG:
			return igbinary_serialize_v2_long(igsd, Z_LVAL_P(z));
		case IS_UNDEF:
			// https://github.com/igbinary/igbinary/issues/134
			// TODO: In a new major version, could have a separate type for IS_UNDEF, which would unset the property in an object context?
			// fallthrough
		case IS_NULL:
			return igbinary_serialize_v2_null(igsd);
		case IS_TRUE:
			return igbinary_serialize_v2_bool(igsd, 1);
		case IS_FALSE:
			return igbinary_serialize_v2_bool(igsd, 0);
		case IS_DOUBLE:
			return igbinary_serialize_v2_double(igsd, Z_DVAL_P(z));
		default:
			zend_error(E_ERROR, "igbinary_serialize_v2_zval: zval has unknown type %d", (int)Z_TYPE_P(z));
			return 1;
	}
...
/* }}} */
/* {{{ igbinary_unserialize_v2_data_init */
/** Inits igbinary_unserialize_data. */
inline static int igbinary_unserialize_v2_data_init(struct igbinary_unserialize_data *igsd) {
	struct igbinary_value_ref *references = emalloc(sizeof(igsd->references[0]) * 4);
	zend_string **strings;
	if (UNEXPECTED(references == NULL)) {
		return 1;
...
/* }}} */
/* {{{ igbinary_unserialize_v2_data_deinit */
/** Deinits igbinary_unserialize_data. */
inline static void igbinary_unserialize_v2_data_deinit(struct igbinary_unserialize_data *igsd) {
	if (igsd->strings) {
		size_t i;
		size_t strings_count = igsd->strings_count;
		zend_string **strings = igsd->strings;
...
/* }}} */
/* {{{ igbinary_unserialize_v2_long */
/** Unserializes zend_long */
inline static int igbinary_unserialize_v2_long(struct igbinary_unserialize_data *igsd, enum igbinary_v2_type t, zend_long *ret) {
	uint32_t tmp32;
#if SIZEOF_ZEND_LONG == 8
	uint64_t tmp64;
#endif

	if (t == igbinary_v2_type_long8p || t == igbinary_v2_type_long8n) {
		if (IGB_NEEDS_MORE_DATA(igsd, 1)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_long: end-of-data");
			return 1;
		}

		*ret = (zend_long)(t == igbinary_v2_type_long8n ? -1 : 1) * igbinary_unserialize8(igsd);
	} else if (t == igbinary_v2_type_long16p || t == igbinary_v2_type_long16n) {
		if (IGB_NEEDS_MORE_DATA(igsd, 2)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_long: end-of-data");
			return 1;
		}

		*ret = (zend_long)(t == igbinary_v2_type_long16n ? -1 : 1) * igbinary_unserialize16(igsd);
	} else if (t == igbinary_v2_type_long32p || t == igbinary_v2_type_long32n) {
		if (IGB_NEEDS_MORE_DATA(igsd, 4)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_long: end-of-data");
			return 1;
		}
		...
		tmp32 = igbinary_unserialize32(igsd);
#if SIZEOF_ZEND_LONG == 4
		if (UNEXPECTED(tmp32 >= 0x80000000 && (tmp32 > 0x80000000 || t == igbinary_v2_type_long32p))) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_long: 64bit long on 32bit platform?");
			tmp32 = 0; /* t == igbinary_v2_type_long32p ? LONG_MAX : LONG_MIN; */
		}
#endif
		*ret = (zend_long)(t == igbinary_v2_type_long32n ? -1 : 1) * tmp32;
	} else {
		ZEND_ASSERT(t == igbinary_v2_type_long64p || t == igbinary_v2_type_long64n);
#if SIZEOF_ZEND_LONG == 8
		if (IGB_NEEDS_MORE_DATA(igsd, 8)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_long: end-of-data");
			return 1;
		...
		/* check for boundaries (perform only one comparison in common case) */
		tmp64 = igbinary_unserialize64(igsd);
		if (UNEXPECTED(tmp64 >= 0x8000000000000000 && (tmp64 > 0x8000000000000000 || t == igbinary_v2_type_long64p))) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_long: too big 64bit long.");
			tmp64 = 0; /* t == igbinary_v2_type_long64p ? LONG_MAX : LONG_MIN */
		}

		*ret = (zend_long)(t == igbinary_v2_type_long64n ? -1 : 1) * tmp64;
#elif SIZEOF_ZEND_LONG == 4
		/* can't put 64bit long into 32bit one, placeholder zero */
		*ret = 0;
		igbinary_unserialize64(igsd);
...
/* }}} */
/* {{{ igbinary_unserialize_v2_double */
/** Unserializes double. */
inline static int igbinary_unserialize_v2_double(struct igbinary_unserialize_data *igsd, double *ret) {
	union {
		double d;
		uint64_t u;
	} u;
	...
	if (IGB_NEEDS_MORE_DATA(igsd, 8)) {
		zend_error(E_WARNING, "igbinary_unserialize_v2_double: end-of-data");
		return 1;
	}
...
/* }}} */
/* {{{ igbinary_unserialize_v2_string */
/** Unserializes string. Unserializes both actual string or by string id. Returns NULL on error. */
inline static zend_string *igbinary_unserialize_v2_string(struct igbinary_unserialize_data *igsd, enum igbinary_v2_type t) {
	size_t i;
	zend_string *zstr;
	if (t == igbinary_v2_type_string_id8 || t == igbinary_v2_type_object_id8) {
		if (IGB_NEEDS_MORE_DATA(igsd, 1)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_string: end-of-data");
			return NULL;
		}
		i = igbinary_unserialize8(igsd);
	} else if (t == igbinary_v2_type_string_id16 || t == igbinary_v2_type_object_id16) {
		if (IGB_NEEDS_MORE_DATA(igsd, 2)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_string: end-of-data");
			return NULL;
		}
		i = igbinary_unserialize16(igsd);
	} else if (t == igbinary_v2_type_string_id32 || t == igbinary_v2_type_object_id32) {
		if (IGB_NEEDS_MORE_DATA(igsd, 4)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_string: end-of-data");
			return NULL;
		}
		...
		zend_error(E_WARNING, "igbinary_unserialize_v2_string: unknown type '%02x', position %zu", t, (size_t)IGB_BUFFER_OFFSET(igsd));
		return NULL;
	}

	if (i >= igsd->strings_count) {
		zend_error(E_WARNING, "igbinary_unserialize_v2_string: string index is out-of-bounds");
		return NULL;
	}
...
/* }}} */
/* igbinary_unserialize_v2_extremely_long_chararray {{{ */
static ZEND_COLD zend_never_inline zend_string* igbinary_unserialize_v2_extremely_long_chararray(struct igbinary_unserialize_data *igsd) {
#if SIZEOF_ZEND_LONG > 4
	zend_error(E_WARNING, "igbinary_unserialize_v2_chararray: cannot unserialize 64-bit data on 32-bit platform");
	return NULL;
#else
	if (IGB_NEEDS_MORE_DATA(igsd, 8)) {
		zend_error(E_WARNING, "igbinary_unserialize_v2_chararray: end-of-data");
		return NULL;
	}
	size_t l = igbinary_unserialize64(igsd);
	if (IGB_NEEDS_MORE_DATA(igsd, l)) {
...
/* }}} */
/* {{{ igbinary_unserialize_v2_chararray */
/** Unserializes chararray of string. Returns NULL on error. */
inline static zend_string *igbinary_unserialize_v2_chararray(struct igbinary_unserialize_data *igsd, enum igbinary_v2_type t, zend_bool check_interned) {
	size_t l;
	zend_string *zstr;

	if (t == igbinary_v2_type_string8 || t == igbinary_v2_type_object8) {
		if (IGB_NEEDS_MORE_DATA(igsd, 1)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_chararray: end-of-data");
			return NULL;
		}
		...
		*/
	} else if (t == igbinary_v2_type_string16 || t == igbinary_v2_type_object16) {
		if (IGB_NEEDS_MORE_DATA(igsd, 2)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_chararray: end-of-data");
			return NULL;
		}
		l = igbinary_unserialize16(igsd);
	} else if (EXPECTED(t == igbinary_v2_type_string32 || t == igbinary_v2_type_object32)) {
		if (IGB_NEEDS_MORE_DATA(igsd, 4)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_chararray: end-of-data");
			return NULL;
		}
		l = igbinary_unserialize32(igsd);
	} else if (t == igbinary_v2_type_string64) {
		return igbinary_unserialize_v2_extremely_long_chararray(igsd);
	} else {
		zend_error(E_WARNING, "igbinary_unserialize_v2_chararray: unknown type '%02x', position %zu", t, (size_t)IGB_BUFFER_OFFSET(igsd));
		return NULL;
	}
	if (IGB_NEEDS_MORE_DATA(igsd, l)) {
		zend_error(E_WARNING, "igbinary_unserialize_v2_chararray: end-of-data");
	...
	if (check_interned && l < 100) {
		 ...
		 * 1. Save memory (e.g. for strings in property default values of classes, for arrays repeating string field names, etc.)
		 * 2. Speed up checking if strings are identical.
		 * 3. Speed up the code that ends up using the return value of igbinary_unserialize_v2().
		 *
		 * Note that this change has mixed results. unserialize-object-array and unserialize-stdclass have performance improve because all strings are already interned,
		 * but unserialize-stringarray on unstructured text has worse performance.
		 */
...
/* }}} */
/* {{{ igbinary_unserialize_v2_array */
/** Unserializes a PHP array. */
zend_always_inline static int igbinary_unserialize_v2_array(struct igbinary_unserialize_data *igsd, enum igbinary_v2_type t, zval *const z, int flags, zend_bool create_ref) {
	/* WANT_REF means that z will be wrapped by an IS_REFERENCE */
	uint32_t n;
	uint32_t i;

	enum igbinary_v2_type key_type;

	HashTable *h;

	if (t == igbinary_v2_type_array8) {
		if (IGB_NEEDS_MORE_DATA(igsd, 1)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_array: end-of-data");
			return 1;
		}
		n = igbinary_unserialize8(igsd);
		if (n == 0) {
	...
	} else if (t == igbinary_v2_type_array16) {
		if (IGB_NEEDS_MORE_DATA(igsd, 2)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_array: end-of-data");
			return 1;
		}
		n = igbinary_unserialize16(igsd);
	} else if (t == igbinary_v2_type_array32) {
		if (IGB_NEEDS_MORE_DATA(igsd, 4)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_array: end-of-data");
			return 1;
		}
		...
		zend_error(E_WARNING, "igbinary_unserialize_v2_array: unknown type '%02x', position %zu", t, (size_t)IGB_BUFFER_OFFSET(igsd));
		return 1;
	}

	/* n cannot be larger than the number of minimum "objects" in the array */
	if (IGB_NEEDS_MORE_DATA(igsd, n)) {
		zend_error(E_WARNING, "igbinary_unserialize_v2_array: data size %zu smaller that requested array length %zu.", (size_t)IGB_REMAINING_BYTES(igsd), (size_t)n);
		return 1;
	}
...
cleanup:
		...
		key_type = (enum igbinary_v2_type)igbinary_unserialize8(igsd);

		switch (key_type) {
			case igbinary_v2_type_long8p:
				/* Manually inline igbinary_unserialize_v2_long() for array keys from 0 to 255, because they're the most common among integers. */
				if (IGB_NEEDS_MORE_DATA(igsd, 1)) {
					zend_error(E_WARNING, "igbinary_unserialize_v2_long: end-of-data");
					goto cleanup;
				}
			...
			case igbinary_v2_type_long16p:
				...
				key_index = igbinary_unserialize16(igsd);
				break;
			case igbinary_v2_type_long8n:
			case igbinary_v2_type_long16n:
			case igbinary_v2_type_long32p:
			case igbinary_v2_type_long32n:
			case igbinary_v2_type_long64p:
			case igbinary_v2_type_long64n:
				if (UNEXPECTED(igbinary_unserialize_v2_long(igsd, key_type, &key_index))) {
					goto cleanup;
				}
				break;
			case igbinary_v2_type_string_id8:
			case igbinary_v2_type_string_id16:
			case igbinary_v2_type_string_id32:
				key_str = igbinary_unserialize_v2_string(igsd, key_type);
				if (UNEXPECTED(key_str == NULL)) {
					goto cleanup;
				}
				break;
			case igbinary_v2_type_string8:
			case igbinary_v2_type_string16:
			case igbinary_v2_type_string32:
			case igbinary_v2_type_string64:
				key_str = igbinary_unserialize_v2_chararray(igsd, key_type, 1);
				if (UNEXPECTED(key_str == NULL)) {
					goto cleanup;
				}
				break;
			case igbinary_v2_type_string_empty:
				key_str = ZSTR_EMPTY_ALLOC();
				break;
			case igbinary_v2_type_null:
				continue;
			default:
				zend_error(E_WARNING, "igbinary_unserialize_v2_array: unknown key type '%02x', position %zu", key_type, (size_t)IGB_BUFFER_OFFSET(igsd));
				goto cleanup;
		}
		...
		ZEND_ASSERT(vp != NULL);
		if (UNEXPECTED(igbinary_unserialize_v2_zval(igsd, vp, WANT_CLEAR))) {
			return 1;
		}
	}
...
/* }}} */
/* {{{ igbinary_unserialize_v2_object_properties */
/** Unserializes the array of object properties and adds those to the object z. */
inline static int igbinary_unserialize_v2_object_properties(struct igbinary_unserialize_data *igsd, enum igbinary_v2_type t, zval *const z, const zend_class_entry *ce) {
	/* WANT_REF means that z will be wrapped by an IS_REFERENCE */
	uint32_t n;
	...
	if (t == igbinary_v2_type_array8) {
		if (IGB_NEEDS_MORE_DATA(igsd, 1)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_object_properties: end-of-data");
			return 1;
		}
		n = igbinary_unserialize8(igsd);
	} else if (t == igbinary_v2_type_array16) {
		...
		zend_error(E_WARNING, "igbinary_unserialize_v2_object_properties: unknown type '%02x', position %zu", t, (size_t)IGB_BUFFER_OFFSET(igsd));
		return 1;
	}

	/* n cannot be larger than the number of minimum "objects" in the array */
	if (IGB_NEEDS_MORE_DATA(igsd, n)) {
		zend_error(E_WARNING, "%s: data size %zu smaller that requested array length %zu.", "igbinary_unserialize_v2_object_properties", (size_t)IGB_REMAINING_BYTES(igsd), (size_t)n);
		return 1;
	}
	...
	do {
		...
		switch (key_type) {
			...
			case igbinary_v2_type_null:
				continue;  /* Skip unserializing this element, serialized with no value. In C, this applies to loop, not switch. */
			default:
				zend_error(E_WARNING, "igbinary_unserialize_v2_object_properties: unknown key type '%02x', position %zu", key_type, (size_t)IGB_BUFFER_OFFSET(igsd));
				zval_dtor(z);
				ZVAL_UNDEF(z);
				return 1;
		}
		...
		if (prototype_value != NULL) {
			...
			/* ZEND_ACC_NO_DYNAMIC_PROPERTIES was introduced in php 8.0 but was largely used with zend_class_unserialize_deny, though external pecls might set this flag. */
			if (UNEXPECTED(ce->ce_flags & ZEND_ACC_NO_DYNAMIC_PROPERTIES)) {
				zend_throw_error(NULL, "Cannot create dynamic property %s::$%s in igbinary_unserialize_v2",
					ZSTR_VAL(ce->name), zend_get_unmangled_property_name(key_str));
				zend_string_release(key_str);
				return 1;
			}
...
/* }}} */
/* {{{ igbinary_unserialize_v2_object_ser */
/** Unserializes object's property array. This is used to serialize objects implementing Serializable -interface. */
static ZEND_COLD int igbinary_unserialize_v2_object_ser(struct igbinary_unserialize_data *igsd, enum igbinary_v2_type t, zval *const z, zend_class_entry *ce) {
	size_t n;
	int ret;
	php_unserialize_data_t var_hash;
	...
	if (t == igbinary_v2_type_object_ser8) {
		if (IGB_NEEDS_MORE_DATA(igsd, 1)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_object_ser: end-of-data");
			return 1;
		}
		n = igbinary_unserialize8(igsd);
	} else if (t == igbinary_v2_type_object_ser16) {
		if (IGB_NEEDS_MORE_DATA(igsd, 2)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_object_ser: end-of-data");
			return 1;
		}
		n = igbinary_unserialize16(igsd);
	} else if (t == igbinary_v2_type_object_ser32) {
		if (IGB_NEEDS_MORE_DATA(igsd, 4)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_object_ser: end-of-data");
			return 1;
		}
		...
		zend_error(E_WARNING, "igbinary_unserialize_v2_object_ser: unknown type '%02x', position %zu", t, (size_t)IGB_BUFFER_OFFSET(igsd));
		return 1;
	}
...
/* }}} */
/* {{{ igbinary_unserialize_v2_object_enum_case */
#if PHP_VERSION_ID >= 80100
/** Unserializes object's property array. This is used to serialize objects implementing Serializable -interface. */
static int igbinary_unserialize_v2_object_enum_case(struct igbinary_unserialize_data *igsd, zval *const z, zend_class_entry *ce) {
	if (UNEXPECTED(!(ce->ce_flags & ZEND_ACC_ENUM))) {
		zend_error(E_WARNING, "igbinary_unserialize_v2_object_enum_case: Class '%s' is not an enum", ZSTR_VAL(ce->name));
		return 1;
	}
	if (IGB_NEEDS_MORE_DATA(igsd, 1)) {
		zend_error(E_WARNING, "igbinary_unserialize_v2_object_enum_case: end-of-data");
		return 1;
	}

	enum igbinary_v2_type t = (enum igbinary_v2_type)igbinary_unserialize8(igsd);
	zend_string *case_name;
	switch (t) {
		case igbinary_v2_type_string8:
		case igbinary_v2_type_string16:
		case igbinary_v2_type_string32:
		case igbinary_v2_type_string64:
			case_name = igbinary_unserialize_v2_chararray(igsd, t, 1);
			break;
		default:
			case_name = igbinary_unserialize_v2_string(igsd, t);
			break;
	}
	if (UNEXPECTED(!case_name)) {
		return 1;
	...
	zval *zv = zend_hash_find(&ce->constants_table, case_name);
	if (UNEXPECTED(!zv)) {
		zend_error(E_WARNING, "igbinary_unserialize_v2_object_enum_case: Undefined constant %s::%s", ZSTR_VAL(ce->name), ZSTR_VAL(case_name));
		zend_string_release(case_name);
		return 1;
	}
	...
	zend_class_constant *c = Z_PTR_P(zv);
	if (UNEXPECTED(!(ZEND_CLASS_CONST_FLAGS(c) & ZEND_CLASS_CONST_IS_CASE))) {
		zend_error(E_WARNING, "igbinary_unserialize_v2_object_enum_case: %s::%s is not an enum case", ZSTR_VAL(ce->name), ZSTR_VAL(case_name));
		zend_string_release(case_name);
		return 1;
	}
	zend_string_release(case_name);
...
#endif
/* }}} */
/* {{{ igbinary_unserialize_v2_object */
/** Unserialize an object.
 * @see ext/standard/var_unserializer.c in the php-src repo. Parts of this code are based on that.
 */
zend_always_inline static int igbinary_unserialize_v2_object(struct igbinary_unserialize_data *igsd, enum igbinary_v2_type t, zval *z, int flags) {
	zend_class_entry *ce;
	...
	if (t == igbinary_v2_type_object8 || t == igbinary_v2_type_object16 || t == igbinary_v2_type_object32) {
		class_name = igbinary_unserialize_v2_chararray(igsd, t, 1);
	} else if (t == igbinary_v2_type_object_id8 || t == igbinary_v2_type_object_id16 || t == igbinary_v2_type_object_id32) {
		class_name = igbinary_unserialize_v2_string(igsd, t);
	} else {
		zend_error(E_WARNING, "igbinary_unserialize_v2_object: unknown object type '%02x', position %zu", t, (size_t)IGB_BUFFER_OFFSET(igsd));
		return 1;
	}
	...
	/* add this to the list of unserialized references, get the index */
	if (IGB_NEEDS_MORE_DATA(igsd, 1)) {
		zend_error(E_WARNING, "igbinary_unserialize_v2_object: end-of-data");
		zend_string_release(class_name);
		return 1;
	}
	...
	t = (enum igbinary_v2_type)igbinary_unserialize8(igsd);
	switch (t) {
		case igbinary_v2_type_array8:
		case igbinary_v2_type_array16:
		case igbinary_v2_type_array32:
		{
			if (UNEXPECTED(object_init_ex(z, ce) != SUCCESS)) {
				php_error_docref(NULL, E_NOTICE, "igbinary unable to create object for class entry");
				r = 1;
			...
			if (incomplete_class) {
...
#endif
				{
					...
					int result;
					zend_string_release(class_name);
					result = igbinary_unserialize_v2_array(igsd, t, &param, 0, false);
					ZVAL_DEREF(z);
					ZEND_ASSERT(Z_TYPE_P(z) == IS_OBJECT);
					igsd_defer_unserialize(igsd, Z_OBJ_P(z), param);
					return result;
			...
			r = igbinary_unserialize_v2_object_properties(igsd, t, z, ce);
			break;
		}
		case igbinary_v2_type_object_ser8:
		case igbinary_v2_type_object_ser16:
		case igbinary_v2_type_object_ser32:
		{
			is_from_serialized_data = true;
			/* FIXME will this break if z isn't an object? */
			r = igbinary_unserialize_v2_object_ser(igsd, t, z, ce);
			if (r != 0) {
				break;
			}
		...
		case igbinary_v2_type_enum_case:
#if PHP_VERSION_ID >= 80100
			if (UNEXPECTED(incomplete_class)) {
				zend_error(E_WARNING, "igbinary_unserialize_v2_object_enum_case: Class '%s' does not exist", ZSTR_VAL(class_name));
				zend_string_release(class_name);
				return 1;
			}
			zend_string_release(class_name);
			r = igbinary_unserialize_v2_object_enum_case(igsd, z, ce);
			if (r) {
				return r;
			}
			...
			return 0;
#else
			zend_error(E_WARNING, "igbinary_unserialize_v2_object: Cannot unserialize enum cases prior to php 8.1 at position %zu", (size_t)IGB_BUFFER_OFFSET(igsd));
			r = 1;
#endif
			break;
		default:
			zend_error(E_WARNING, "igbinary_unserialize_v2_object: unknown object inner type '%02x', position %zu", t, (size_t)IGB_BUFFER_OFFSET(igsd));
			r = 1;
	}
	zend_string_release(class_name);
	class_name = NULL;
	...
	if (r == 0 && !is_from_serialized_data) {
		...
		if (ref->type == IG_REF_IS_OBJECT) {
			...
			zval ztemp = ref->reference.reference->val;
			if (UNEXPECTED(Z_TYPE(ztemp) != IS_OBJECT)) {
				zend_error(E_WARNING, "igbinary_unserialize_v2_object preparing to __wakeup/__unserialize: got reference to non-object somehow (inner type '%02x', position %zu)", t, (size_t)IGB_BUFFER_OFFSET(igsd));
				return 1;
			}
			object = Z_OBJ(ztemp);
		} else {
			zend_error(E_WARNING, "igbinary_unserialize_v2_object preparing to __wakeup/__unserialize: created non-object somehow (inner type '%02x', position %zu)", t, (size_t)IGB_BUFFER_OFFSET(igsd));
			return 1;
		}
		if (object->ce != PHP_IC_ENTRY) {
			if (zend_hash_str_exists(&object->ce->function_table, "__wakeup", sizeof("__wakeup") - 1)) {
...
/* }}} */
/* {{{ igbinary_unserialize_v2_ref */
/** Unserializes an array or object by reference. */
zend_always_inline static int igbinary_unserialize_v2_ref(struct igbinary_unserialize_data *igsd, enum igbinary_v2_type t, zval *const z, int flags) {
	size_t n;

	if (t == igbinary_v2_type_ref8 || t == igbinary_v2_type_objref8) {
		if (IGB_NEEDS_MORE_DATA(igsd, 1)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_ref: end-of-data");
			return 1;
		}
		n = igbinary_unserialize8(igsd);
	} else if (t == igbinary_v2_type_ref16 || t == igbinary_v2_type_objref16) {
		if (IGB_NEEDS_MORE_DATA(igsd, 2)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_ref: end-of-data");
			return 1;
		}
		n = igbinary_unserialize16(igsd);
	} else if (t == igbinary_v2_type_ref32 || t == igbinary_v2_type_objref32) {
		if (IGB_NEEDS_MORE_DATA(igsd, 4)) {
			zend_error(E_WARNING, "igbinary_unserialize_v2_ref: end-of-data");
			return 1;
		}
		...
		zend_error(E_WARNING, "igbinary_unserialize_v2_ref: unknown type '%02x', position %zu", t, (size_t)IGB_BUFFER_OFFSET(igsd));
		return 1;
	}

	if (n >= igsd->references_count) {
		zend_error(E_WARNING, "igbinary_unserialize_v2_ref: invalid reference %zu >= %zu", (size_t)n, (size_t)igsd->references_count);
		return 1;
	}
	 ...
	 * TODO: Can there properly be multiple reference groups to an object?
	 * Similar to https://github.com/php/php-src/blob/master/ext/standard/var_unserializer.re , for "R:"
	 * Using `flags` because igbinary_unserialize_v2_ref might be used both for copy on writes ($a = $b = [2]) and by PHP references($a = &$b).
	 */
	if ((flags & WANT_REF) != 0) {
		/* Want to create an IS_REFERENCE, not just to share the same value until modified. */
		switch (ref.type) {
...
/* }}} */
/* {{{ igbinary_unserialize_v2_zval */
/** Unserialize a zval of any serializable type (zval is PHP's internal representation of a value). */
static int igbinary_unserialize_v2_zval(struct igbinary_unserialize_data *igsd, zval *const z, int flags) {
	enum igbinary_v2_type t;
	...
	if (IGB_NEEDS_MORE_DATA(igsd, 1)) {
		zend_error(E_WARNING, "igbinary_unserialize_v2_zval: end-of-data");
		return 1;
	}
	...
	switch (t) {
		case igbinary_v2_type_ref:
			if (UNEXPECTED(igbinary_unserialize_v2_zval(igsd, z, WANT_REF))) {
				return 1;
			}
			...
			switch (type) {
				...
				case IS_TRUE:
				{
					...
					ref.reference.reference = Z_REF_P(z);
					ref.type = IG_REF_IS_REFERENCE;
					/* add the unserialized scalar to the list of unserialized references. Objects and arrays were already added in igbinary_unserialize_v2_zval. */
					RETURN_1_IF_NON_ZERO(igsd_append_ref(igsd, ref) == SIZE_MAX);
					break;
				}
				default:
			...
			break;
		case igbinary_v2_type_objref8:
		case igbinary_v2_type_objref16:
		case igbinary_v2_type_objref32:
		case igbinary_v2_type_ref8:
		case igbinary_v2_type_ref16:
		case igbinary_v2_type_ref32:
			if (UNEXPECTED(igbinary_unserialize_v2_ref(igsd, t, z, flags))) {
				return 1;
			}
			break;
		case igbinary_v2_type_object8:
		case igbinary_v2_type_object16:
		case igbinary_v2_type_object32:
		case igbinary_v2_type_object_id8:
		case igbinary_v2_type_object_id16:
		case igbinary_v2_type_object_id32:
			if (UNEXPECTED(igbinary_unserialize_v2_object(igsd, t, z, flags))) {
				return 1;
			}
			break;
		case igbinary_v2_type_array8:
		case igbinary_v2_type_array16:
		case igbinary_v2_type_array32:
			if (UNEXPECTED(igbinary_unserialize_v2_array(igsd, t, z, flags, true))) {
				return 1;
			}
			break;
		case igbinary_v2_type_string_empty:
		...
		case igbinary_v2_type_string_id16:
		case igbinary_v2_type_string_id32:
			tmp_str = igbinary_unserialize_v2_string(igsd, t);
			if (UNEXPECTED(tmp_str == NULL)) {
				return 1;
			}
			ZVAL_STR(z, tmp_str);
		...
		case igbinary_v2_type_string32:
		case igbinary_v2_type_string64:
			tmp_str = igbinary_unserialize_v2_chararray(igsd, t, 0);
			if (UNEXPECTED(tmp_str == NULL)) {
				return 1;
			}
			ZVAL_STR(z, tmp_str);
			break;
		case igbinary_v2_type_long8p:
			/* Manually inline igbinary_unserialize_v2_long() for values from 0 to 255, because they're the most common among integers in many applications. */
			if (IGB_NEEDS_MORE_DATA(igsd, 1)) {
				zend_error(E_WARNING, "igbinary_unserialize_v2_long: end-of-data");
				return 1;
			}
		...
		case igbinary_v2_type_long64p:
		case igbinary_v2_type_long64n:
			if (UNEXPECTED(igbinary_unserialize_v2_long(igsd, t, &tmp_long))) {
				return 1;
			}
			ZVAL_LONG(z, tmp_long);
			break;
		case igbinary_v2_type_null:
			ZVAL_NULL(z);
			break;
		case igbinary_v2_type_bool_false:
			ZVAL_BOOL(z, 0);
			break;
		case igbinary_v2_type_bool_true:
			ZVAL_BOOL(z, 1);
			break;
		case igbinary_v2_type_double:
			if (UNEXPECTED(igbinary_unserialize_v2_double(igsd, &tmp_double))) {
				return 1;
			}
			ZVAL_DOUBLE(z, tmp_double);
			break;
		default:
			zend_error(E_WARNING, "igbinary_unserialize_v2_zval: unknown type '%02x', position %zu", t, (size_t)IGB_BUFFER_OFFSET(igsd));
			return 1;
	}
